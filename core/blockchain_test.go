...
func TestGetReceiptByIndex(t *testing.T) {
	...
	for i := 0; i < chainLength; i++ {
		block := blocks[i]
		rawReceipts := chain.GetReceiptsByHash(block.Hash())
		chain.receiptsCache.Purge() // ugly hack

		var blobGasPrice *big.Int
		if block.ExcessBlobGas() != nil {
			blobGasPrice = eip4844.CalcBlobFee(chain.chainConfig, block.Header())
		}
		err := rawReceipts.DeriveFields(chain.chainConfig, block.Hash(), block.NumberU64(), block.Time(), block.BaseFee(), blobGasPrice, block.Transactions())
		if err != nil {
		...
		for txIndex, tx := range block.Body().Transactions {
			...
			if !reflect.DeepEqual(rawReceipts[txIndex], receipt) {
				want := spew.Sdump(rawReceipts[txIndex])
				got := spew.Sdump(receipt)
				t.Fatalf("Receipt is not matched, want %s, got: %s", want, got)
			}
		}
