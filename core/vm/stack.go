...
func (st *Stack) pop5(peekLastN uint) (*uint256.Int, *uint256.Int, *uint256.Int, *uint256.Int, *uint256.Int, error) {
	const requiredDepth = 5
	depth := st.len()
	if depth < requiredDepth {
		return nil, nil, nil, nil, nil, ErrStackUnderflow{st.len(), requiredDepth}
	}
	stack := st.data
	st.data = st.data[:depth-requiredDepth]
	st.growUnsafeByN(peekLastN)
	return &stack[depth-1], &stack[depth-2], &stack[depth-3], &stack[depth-4], &stack[depth-5], nil
}
